...
`readSession` applies the same session lifetime as the probe. It destroys a
session past `sessionMaxAgeMs` and clears its cookie. It sets no caching policy,
so send a page rendered from its answer with a private or `no-store`
`Cache-Control`.
