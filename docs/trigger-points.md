...
| Trigger point      | Symbol                                                    | Fires                                                                                | Required                                  | Control                  | On throw                                                                                                                                                                                                                                                                                                                                 |
| ------------------ | --------------------------------------------------------- | ------------------------------------------------------------------------------------ | ----------------------------------------- | ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| user-store         | `IdentityUserStore<User>` (`users`)                       | Every flow (`create` in `signUp`, `findByIdentifier`/`getCredential` in `signIn`, …) | yes                                       | blocks                   | Propagates and aborts the flow. **Throwing from `create` is the only pre-registration veto** (see Known gaps). `markEmailVerified` runs _before_ the token is consumed, so a throw leaves the verification link retryable                                                                                                                |
...
| token-flow-store   | `TokenFlowStore` (via `tokens`)                           | All `request*` / `verify*` / `reset*` / `unlock*` / `consume*` flows                 | required for those flows                  | blocks                   | Propagates and aborts the flow — except while minting in `requestPasswordReset` / `requestSignInLink`, where a throw is trapped like a delivery failure: the call resolves `void`, nothing is sent, and `credential_mint.failed` is the only signal (a store outage is not an enumeration oracle)                                        |
| otp-store          | `OtpStore` (`otp.store`)                                  | `requestSignInCode` / `verifySignInCode`                                             | required for the code flow                | blocks                   | Propagates and aborts the flow — except while minting in `requestSignInCode`, trapped as for token-flow-store (`credential_mint.failed`)                                                                                                                                                                                                 |
| session-revocation | `RevocableSessionService` (`sessions`)                    | `resetPassword`, after the new credential is set and before the token is consumed    | no                                        | blocks (side-effect)     | A throw (after the password change) emits `password_reset.failed` (`session_revocation_failed`), suppresses `password_reset.completed`, and rethrows; the reset link is left unspent so the user finishes the reset by submitting it again — reported failed and retryable, not silently half-complete                                   |
...
| profile-mapper    | `OAuth2ProfileMapper` (`mapProfile`)        | External callback, mapping the raw profile                             | with `oauth2Provider`                 | blocks  | A missing/empty `subject` in the _return_ → `provider_error`; a mapper that _throws_ surfaces raw |
...
| rate-limiter  | `RateLimiterLike` (`rateLimiter`) | Start of each `verify` and `confirmEnrollment` (`check`); reset on success | no       | blocks          | A `check` that _throws_ → `rate_limited` under `enforce` (fail closed), logged-and-allowed under `log-only`; a limit _hit_ under `enforce` → `rate_limited`. A `reset` that throws is logged and never fails the verification — the code is already spent by then |
...
| resolve-user             | `resolveUser` (`HonoBffOptions`)              | `/auth/callback`, enriching session user claims   | no                       | blocks  | **Relabeled as `invalid_grant` "code exchange failed"** even though it runs post-exchange (see Failure behavior) |
| callback-error           | `onCallbackError` (`HonoBffOptions`)          | `/auth/callback` cannot complete                  | no (default 400 JSON)    | blocks  | **Invoked with no guard — its own throw propagates to Hono (500)** (see Failure behavior)                        |
